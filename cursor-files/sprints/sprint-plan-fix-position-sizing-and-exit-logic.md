...
**Current Status**: Short positions are over-leveraged (5× bias at 0.8-0.9), exits allow optimistic fallback to mid-price, end-of-game closes ignore liquidity collapse, and missing realism filters allow noise trading  
**Target Status**: Risk-neutral position sizing for shorts, symmetric exit requirements (or slippage penalty), realistic end-of-game closes, and realism filters to reduce noise trading  
...
- **QC Results**: N/A
- **Code Coverage**: N/A
- **Build Status**: Application runs successfully, but simulation has correctness issues
...
### Business Context
- **Business Driver**: Current simulation has critical correctness issues that bias results toward profitability and understate risk. Short positions are over-leveraged (5× bias), exits are optimistic (allow fallback to mid-price), and end-of-game closes ignore liquidity collapse. Missing realism filters allow noise trading that doesn't reflect real trading constraints.
- **Success Criteria**: 
  - Short position sizing uses risk-neutral formula (bet_amount / (1 - entry_price))
  - Exit logic requires bid/ask OR applies slippage penalty for fallback
  - End-of-game forced closes apply slippage penalty
  - Minimum holding period enforced (30-60 seconds)
  - Hysteresis exit logic prevents churn
  - ESPN direction confirmation filters noise entries
  - Game phase stratification provides strategic insights
  - All cached results invalidated (old results biased)
...
- **Current System State**: 
  - Short position sizing: `bet_amount / entry_price` (WRONG - over-leverages shorts)
  - Exit logic: Allows fallback to mid-price (optimistic)
  - End-of-game close: Uses market price without slippage (optimistic)
  - No minimum holding period (allows noise trading)
  - No hysteresis exit (allows churn)
  - No direction confirmation (allows noise entries)
  - No game phase stratification
- **Target System State**: 
  - Short position sizing: `bet_amount / (1 - entry_price)` (risk-neutral)
  - Exit logic: Requires bid/ask OR applies slippage penalty (realistic)
  - End-of-game close: Applies forced slippage penalty (realistic)
  - Minimum holding period: 30-60 seconds (configurable)
  - Hysteresis exit: Only exit when divergence crosses from outside threshold
  - Direction confirmation: Only enter when divergence is widening
  - Game phase stratification: Q1, Q2-Q3, Q4 breakdown
...
- **In Scope**: 
  ...
  - Fix exit logic asymmetry (require bid/ask OR apply slippage)
  - Fix end-of-game forced close (apply slippage penalty)
...
- **Assumptions**: 
  ...
  - Exit fallback slippage: 1-2 cents (conservative estimate)
  - All cached results should be invalidated (old results biased)
- **Constraints**: 
...
**Objective**: Require bid/ask for exit OR apply slippage penalty for fallback
...
**Deliverables**: Updated end-of-game close logic with slippage
...
**Priority**: Critical (fixes fundamental bias)
...
- **Acceptance Criteria**:
  ...
  - [ ] Max loss calculation correct for shorts (max loss = bet_amount when price goes to $1.00)

- **Technical Context**:
  - **Current State**: 
    ```python
    # Short position sizing (WRONG)
...
- **Implementation Steps**:
  ...
  3. Update comment to explain risk-neutral sizing
  4. Add unit tests for short sizing at various entry prices (0.1, 0.5, 0.8, 0.9)
  5. Verify max loss calculation is correct
...
- **Validation Steps**:
  ...
  # Test max loss (price goes to 1.00)
  exit_price = 1.00
...
- **Definition of Done**: 
  ...
  - Max loss equals bet_amount for shorts
  - Unit tests pass
...
- **Success Metrics**:
  ...
  - **Quality**: Max loss equals bet_amount for shorts
  - **Performance**: No performance degradation (same O(1) complexity)

---

### Epic 2: Fix Exit Logic Asymmetry
**Priority**: High (fixes optimistic execution assumption)
**Estimated Time**: 2-3 hours
...
- **Files to Modify**: 
  ...
  - `scripts/simulate_trading_strategy.py` (function: `calculate_trade_pnl()` - add fallback slippage parameter)
- **Files to Create**: None
- **Dependencies**: None

- **Acceptance Criteria**:
  - [ ] Exit logic applies slippage penalty when using mid-price fallback
  - [ ] Slippage penalty: 1-2 cents (configurable, default 1.5 cents)
...
**Priority**: High (fixes optimistic late-game execution)
...
- **Acceptance Criteria**:
  ...
  - [ ] Long positions: Subtract slippage from final_bid
  - [ ] Short positions: Add slippage to final_ask
...
**Priority**: Medium-High (improves realism, reduces noise)
...
- **Acceptance Criteria**:
  ...
  - [ ] Track `prev_abs_divergence` in simulation state
  - [ ] Exit condition: `abs_divergence < exit_threshold AND prev_abs_divergence >= exit_threshold`
...
- **Technical Context**:
  ...
  - **Data Structures**: Add `prev_abs_divergence` to `SimulationState`
  - **API Contracts**: No API changes (internal logic)

- **Implementation Steps**:
  1. Add `prev_abs_divergence` to `SimulationState` dataclass
  2. Update exit logic to check `prev_abs_divergence >= exit_threshold`
  3. Update `prev_abs_divergence` after each data point
...
- **Acceptance Criteria**:
  ...
  - [ ] Long entry: `divergence > entry_threshold AND divergence > prev_divergence`
  - [ ] Short entry: `divergence < -entry_threshold AND divergence < prev_divergence`
...
- **Technical Context**:
  ...
  - **Required Changes**: 
    ...
    elif divergence < -entry_threshold and divergence < prev_divergence:
        # Enter short (divergence is widening, not shrinking)
    ```
  - **Integration Points**: Entry logic in `simulate_trading_strategy()`
  - **Data Structures**: Use `prev_divergence` from `SimulationState` (already added for hysteresis)
  - **API Contracts**: No API changes (internal logic)

- **Implementation Steps**:
  1. Update entry logic to check `divergence > prev_divergence` (long) or `divergence < prev_divergence` (short)
  2. Track `prev_divergence` in simulation state (already added for hysteresis)
...
- **Problem**: Current short sizing over-leverages shorts near 0.8-0.9, creating 5× bias
...
- **Over-Engineering Risk**: None
- **Selected**: ❌ **REJECTED** - Maintains 5× bias

**Chosen Solution**: Risk-neutral sizing for shorts
- **Rationale**: Binary markets have asymmetric max loss (long: entry_price, short: 1-entry_price). Risk-neutral sizing ensures both positions have same maximum risk.
...
- **Risk Assessment**: ✅ Accurate max loss calculation
...
- **Problem**: Entries require bid/ask, but exits allow fallback to mid-price (optimistic)
...
- **Algorithm**: Apply 1-2 cent slippage penalty when using mid-price fallback
- **Implementation Complexity**: Low
- **Maintenance Overhead**: Low
- **Scalability**: Excellent
...
- **Mitigation**: Use conservative estimate (1-2 cents), document as assumption
...
1. **Short Position Sizing**:
   ...
   - Verify max loss equals bet_amount
...
- **Rollback Plan**: 
  - Revert code changes
  - Restore old sizing (not recommended - old results biased)
